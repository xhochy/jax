...
def eigh(
  ...
  Args:
    ...
    sort_eigenvalues: If ``True``, the eigenvalues will be sorted in ascending
     ...
     select_range: Optional 2-tuple [start, end] indicating the range of indices
       of eigenvalues to compute. For example, is ``range_select`` = [n-2,n],
       then ``eigh`` computes the two largest eigenvalues and their
       eigenvectors.
  ...
  Returns:
    ...
    If ``select_range`` is ``None`` then ``d`` is equal to ``n``. Otherwise
    ``d`` is equal to ``select_Range[1] - select_range[0]``.
  """
  if symmetrize_input:
    x = symmetrize(x)
  v, w = eigh_p.bind(
...
def _eigh_impl(operand, *, lower, sort_eigenvalues, select_range):
...
def _eigh_abstract_eval(operand, *, lower, sort_eigenvalues, select_range):
  if isinstance(operand, ShapedArray):
    if operand.ndim < 2 or operand.shape[-2] != operand.shape[-1]:
      raise ValueError(
        "Argument to symmetric eigendecomposition must have shape [..., n, n],"
    ...
    batch_dims = operand.shape[:-2]
    n = operand.shape[-1]
    d = n if select_range is None else select_range[1] - select_range[0]
...
def _eigh_cpu_gpu_lowering(
    syevd_impl, ctx, operand, *, lower, sort_eigenvalues, select_range
):
  ...
  if not (select_range is None or select_range == (0, n)):
    raise NotImplementedError(f"select_range not implemented for CPU and GPU")
...
def _eigh_tpu_impl(x, *, lower, sort_eigenvalues, select_range):
  *_, m, n = x.shape
  assert m == n, (m, n)
  ...
  if m <= termination_size and (select_range is None or select_range == (0, n)):
...
def _eigh_jvp_rule(primals, tangents, *, lower, sort_eigenvalues, select_range):
  (a,) = primals
  n = a.shape[-1]
  if not (select_range is None or select_range == (0, n)):
    raise NotImplementedError(
...
def _eigh_batching_rule(
    batched_args, batch_dims, *, lower, sort_eigenvalues, select_range
):
  x, = batched_args
  bd, = batch_dims
  x = batching.moveaxis(x, bd, 0)
